...
class Krs extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'krs/index?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'krs/index?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'krs/index';
            $config['first_url'] = base_url() . 'krs/index';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Krs_model->total_rows($q);
        $krs = $this->Krs_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $this->load->view('mahasiswa/krs_list', $data);
    }
